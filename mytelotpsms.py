...
# ------------------ Configuration ------------------
BOT_TOKEN = "8763208647:AAFK8k0QxetI-zY8TRrv4NC4TkEd5LkpuA8"
...
def main():
    ...
    single_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^📱 Request OTP$"), single_otp_start_simple),
    ...
    validate_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^✅ Validate OTP$"), validate_otp_start_simple),
    ...
    account_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^📄 Account Details$"), account_details_start_simple),
    ...
    schedule_type_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^⏰ Schedule OTP$"), schedule_type_selection),
    ...
    advanced_schedule_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^📅 Advanced Schedule$"), advanced_schedule_start),
    ...
    mass_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^🔄 Mass OTP \(250x\)$"), mass_otp_start),
    ...
    fifty_x_three_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^⚡ 50x3 \(50 codes/3min\)$"), fifty_x_three_start),
    ...
    app.add_handler(MessageHandler(filters.Regex("^(📱 Request OTP|✅ Validate OTP|📄 Account Details|⏰ Schedule OTP|📅 Advanced Schedule|🔄 Mass OTP \(250x\)|⚡ 50x3 \(50 codes/3min\)|🛑 Stop Task|❓ Help|🔘 Inline Menu)$"), handle_simple_buttons))
